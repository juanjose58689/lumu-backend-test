...
def main():
    ...
    parser.add_argument(
        "--topic", required=True, help="Kafka topic to send messages to"
    )
    parser.add_argument(
        "--bootstrap_servers", default="localhost:9092", help="Kafka bootstrap servers"
    )
    parser.add_argument(
        "--num_devices", type=int, default=100, help="Number of devices to simulate"
    )
    parser.add_argument(
        "--pause_ms",
        type=int,
        default=100,
        help="Pause between messages in milliseconds",
    )
    parser.add_argument(
        "--num_messages",
        type=int,
